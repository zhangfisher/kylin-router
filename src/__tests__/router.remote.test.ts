...
describe("Remote route loading", () => {
    ...
    it("routes 配置为函数时应该调用函数返回路由配置", async () => {
        ...
        expect(router.currentRoute!.route.name).toBe("about");
    });
    ...
    it("routes 配置为异步函数时应该 await 后返回路由配置", async () => {
        ...
        expect(router.currentRoute).not.toBeNull();
        expect(router.currentRoute!.route.name).toBe("dashboard");
    });
    ...
    it("loadRemoteRoutes() 应该合并远程路由到现有路由表", async () => {
        ...
        router.push("/profile");
        expect(router.currentRoute!.route.name).toBe("profile");
    });
    ...
    it("loadRemoteRoutes() 支持传入函数", async () => {
        ...
        router.push("/reports");
        expect(router.currentRoute!.route.name).toBe("reports");
    });
