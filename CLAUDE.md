...
- **路由特性**： KylinRouter是核心路由入口，相关的特性实现统一保存在src/features，特性通过两种方式进行扩展实现：
      - 使用ts-mixer通过Mixin的方式进行功能扩展,但是Mixin最多只支持10个特性，超过10个特性需要使用第二种方式进行功能扩展。在 Mixin 中，内部辅助方法应该使用 protected 修饰符，只有公共 API 才使用 public。内部方法应该使用private修饰并以_ 前缀标识。 私有属性应声明在KylinRouter类中，并使用protected和_ 前缀来标识，如 _contextRequested 和 _cleanups。公开属性应使用 public 修饰符，并且不使用 _ 前缀。
...
- **示例**：保存在 `example/` 目录
- **事件系统**: 通过KylinRouter实例上的emit方法触发，事件类型定义在KylinRouterEvents中，事件监听通过KylinRouter实例上的on方法进行监听
- 当与实际代码不一致时，以实际代码为准，并要及时更新计划内容。
