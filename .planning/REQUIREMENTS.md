...
- [x] **LOAD-01**: 路由器支持远程 HTML 加载，通过 fetch URL 获取内容
- [x] **LOAD-02**: 路由器使用 lit/html 渲染远程 HTML 到 outlet
- [x] **LOAD-03**: 路由器在组件加载失败时显示错误边界组件
- [x] **LOAD-04**: 路由器实现自动重试机制，加载失败时重试加载
- [x] **LOAD-05**: 路由器提供 `<kylin-loading>` 组件显示加载状态

### 用户体验 (UX)

- [ ] **UX-01**: 路由器支持转场动画，路由切换时显示过渡效果
- [ ] **UX-02**: 路由器基于 View Transitions API 实现动画效果
- [x] **UX-03**: 路由器提供加载状态显示组件，提升用户体验
...
- [x] **MODAL-01**: 路由器支持模态路由渲染，在 host 元素下的 `<div class="kylin-modals">` 中打开模态框
- [x] **MODAL-02**: 模态路由支持背景遮罩和关闭交互
- [x] **MODAL-03**: 模态路由可以与普通路由共存，支持多层模态

### 错误处理 (ERROR)

- [x] **ERROR-01**: 路由器实现全局错误边界，捕获组件渲染错误
- [x] **ERROR-02**: 路由器在导航竞态条件时丢弃旧响应，避免状态混乱
