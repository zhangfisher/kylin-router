...
**设计决策：**
- **Mixin 架构** - 每个特性独立为一个 Mixin 类，在 `KylinRouter` 中组合。如果Mixin类超出10个，则使用管理器方式来实现特性组合。
- **Outlet 模式** - 通过 `<kylin-outlet>` 组件声明路由渲染位置
- **上下文传播** - 组件继承自KylinRouterElementBase，通过 向上查找data-kylin-router和context-request 事件获取路由实例
