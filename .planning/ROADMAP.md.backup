...
- [ ] **Phase 1.1: Refactor Routes（代码重构）** - 消除 router.ts 与 Routes mixin 的代码重复
...
**Plan List:**
- [ ] 1.1-01-PLAN.md — 重构 router.ts，移除与 Routes mixin 重复的代码
...
**Success Criteria** (what must be TRUE):
  ...
  2. 路由器支持 beforeResolve 钩子，在组件解析前执行数据预取
  3. 路由器支持 afterEach 钩子，在导航完成后执行清理逻辑
  4. 路由守卫支持异步操作（权限验证、数据预加载）
  5. 路由守卫可以取消导航或重定向到其他路由
  6. 路由器支持多层嵌套路由，路径自动继承和匹配
  7. 用户可以通过 `<kylin-outlet>` 组件声明路由渲染位置
  8. 路由器支持多个 outlet 元素（命名 outlet），同一路由可渲染到多个位置
  9. 路由切换时自动滚动到目标 outlet 位置，提升用户体验
...
| 1.1. Refactor Routes（代码重构） | 0/1 | Not started | - |
