...
- **Global scope**: `C:/Work/Code/kylin-router/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Work/Code/kylin-router/.claude/notes/`)
...
2. Glob `C:/Work/Code/kylin-router/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Work/Code/kylin-router/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Work/Code/kylin-router/.claude/notes/` — works in any directory
